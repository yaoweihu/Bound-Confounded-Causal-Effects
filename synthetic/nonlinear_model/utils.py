...
def gen_linear_data(data_size, is_x=None):
    ...
    z = np.linspace(0, 6, data_size).reshape(data_size, 1) + uz
    x = 0.8 * z + 5.0 * u1 + ux if is_x is None else is_x
    w = 0.7 * x + 1.0 * u2 + uw
    v = 0.2 * u1 + 0.4 * u2 + uv
    y = 1.8 * w + 0.7 *v + uy
    return np.concatenate((x, y, z, w, v), axis=1)

def gen_nonlinear_data(data_size, is_x=None):
    uz = 0.1 * np.random.randn(data_size, 1)
    ux = 0.1 * np.random.randn(data_size, 1)
    uv = 0.1 * np.random.randn(data_size, 1)
    uw = 0.1 * np.random.randn(data_size, 1)
    ...
    u1 = 0.1 * np.random.randn(data_size, 1)
    u2 = 0.1 * np.random.randn(data_size, 1)
    z = np.linspace(-3, 3, data_size).reshape(data_size, 1) + uz
    x = -0.5 + 0.8 * z + 2.0 * u1 + ux if is_x is None else is_x
    w = 0.7 * x * x + 0.3 * u2 + uw
    v = 0.2 * u1 + 0.4 * u2 + uv
    y = 0.6 * w + 0.7 *v + uy
    return np.concatenate((x, y, z, w, v), axis=1)
